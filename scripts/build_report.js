...
function openPeriodModal(type) {
  ...
  var yDate = new Date(yd.date + 'T00:00:00');
  var isWeek = (type === 'week');

  var startDate, title, icon;
  if (isWeek) {
    var dow = yDate.getDay();
    var sun = new Date(yDate);
    sun.setDate(yDate.getDate() - dow);
    startDate = sun.toISOString().slice(0, 10);
    title = 'Revenue This Week (Sun–Today)';
    icon = '📅';
  } else {
    startDate = yd.date.slice(0, 7) + '-01';
    title = 'Revenue This Month (1st–Today)';
    icon = '🗓️';
  }
  var endDate = yd.date;
  ...
  pDays.forEach(function(d) {
    var dayTotRev = (d.totalRevenue || 0) + ((d.deliveryFee && d.deliveryFee.total) || 0);
    totalRevenue += dayTotRev;
    totalDeals += (d.salesCount || 0);

    var dirRev = (d.directSaleCSV && d.directSaleCSV.revenue) || 0;
    var dirCnt = (d.directSaleCSV && d.directSaleCSV.count) || 0;
    totalDirectRev += dirRev;
    totalDirectCnt += dirCnt;

    var delRev = (d.deliveryFee && d.deliveryFee.total) || 0;
    var delCnt = (d.deliveryFee && d.deliveryFee.count) || 0;
    totalDelFeeRev += delRev;
    totalDelFeeCnt += delCnt;
  ...
  var totalInsideRev = totalRevenue - totalDirectRev - totalDelFeeRev;
  var totalInsideCnt = Math.max(0, totalDeals - totalDirectCnt);
  ...
  pDays.forEach(function(d) {
    var dTot = (d.totalRevenue || 0) + ((d.deliveryFee && d.deliveryFee.total) || 0);
    var dDir = (d.directSaleCSV && d.directSaleCSV.revenue) || 0;
    var dDel = (d.deliveryFee && d.deliveryFee.total) || 0;
    var dIns = dTot - dDir - dDel;
    ...
    outHtml += '<tr>' +
      ...
      '<td style="text-align: right; color: var(--green); font-weight: 700;">' + fmt(dIns) + '</td>' +
...
function exportPeriodPDF(title, startDate, endDate) {
  ...
  var body = pDays.map(function(d) {
    ...
    return [
      ...
      '₹' + Math.round(dIns).toLocaleString('en-IN'),
