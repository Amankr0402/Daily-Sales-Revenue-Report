  ...
  <div class="sec-gap">
    ...
    <div class="grid g2">
      ...
      <div class="chart-card" style="padding: 18px 16px;">
        <div style="font-size: 12.5px; font-weight: 800; color: var(--muted); text-align: center; text-transform: uppercase; letter-spacing: 0.05em;">
  ...
  <div class="sec-gap">
    ...
    <div class="grid" style="grid-template-columns:1fr 1fr;gap:14px">
      ...
      <div class="chart-card" style="padding:0;overflow:hidden;display:flex;flex-direction:column;justify-content:space-between">
        ...
        <!-- Leads Missed (>8 hrs, no activity) -->
        <div style="margin: 12px 16px 16px; padding: 14px 18px; background: #ffffff; border: 1px solid var(--border); border-radius: 12px; text-align: center; box-shadow: 0 2px 8px rgba(0,0,0,0.02);">
          <div style="font-size: 26px; line-height: 1; margin-bottom: 6px;">⏰</div>
          <div style="font-size: 13.5px; font-weight: 800; color: var(--text); margin-bottom: 12px;">
            Leads Missed (&gt;8 hrs, no activity)
          </div>
...
async function init() {
  try {
    ...
    // Calculate Monthly Top Agents (MTD)
    ...
    mData.forEach(d => {
      Object.entries(d.agents || {}).forEach(([name, info]) => {
        if (!monthAgentsMap[name]) {
          monthAgentsMap[name] = { revenue: 0, count: 0 };
        }
        monthAgentsMap[name].revenue += (info.revenue || 0);
        monthAgentsMap[name].count += (info.count || 0);
      });
    });
    const ag = Object.entries(monthAgentsMap).sort((a, b) => b[1].revenue - a[1].revenue);
    ...
    // Podium (Top 3 Agents of This Month)
    ...
    // Podium (Bottom 3 Agents of This Month)
    const bottom3 = ag.length >= 3 ? ag.slice(-3) : [];
    const pwBottom = document.getElementById('podium-bottom');
    if (pwBottom) {
      if (!bottom3.length) {
        pwBottom.innerHTML = '<div style="color:#8896b3;text-align:center;padding:20px 0;">No agent data.</div>';
      } else {
    ...
    if (ml) {
      ...
      if (mlTarget) {
        mlTarget.innerHTML = '<table role="presentation" width="100%" cellspacing="0" cellpadding="0" border="0" style="margin-top:14px; padding-top:14px; border-top:1px solid #f1f5f9; table-layout:fixed;"><tr>' +
          '<td valign="top" align="left" style="padding:0 6px"><div style="font-size:11px;text-transform:uppercase;letter-spacing:0.06em;color:#64748b;font-weight:700">CONNECTED</div><div style="font-size:20px;font-weight:900;color:#0f172a;margin-top:5px">' + (ml['Connected'] || 0).toLocaleString('en-IN') + '</div></td>' +
          '<td valign="top" align="left" style="padding:0 6px"><div style="font-size:11px;text-transform:uppercase;letter-spacing:0.06em;color:#64748b;font-weight:700">INTERESTED</div><div style="font-size:20px;font-weight:900;color:#0f172a;margin-top:5px">' + (ml['Interested'] || 0).toLocaleString('en-IN') + '</div></td>' +
          '<td valign="top" align="left" style="padding:0 6px"><div style="font-size:11px;text-transform:uppercase;letter-spacing:0.06em;color:#64748b;font-weight:700">PAISE DEGA</div><div style="font-size:20px;font-weight:900;color:#0f172a;margin-top:5px">' + (ml['Paise Dega'] || 0).toLocaleString('en-IN') + '</div></td>' +
          '<td valign="top" align="right" style="padding:0 6px"><div style="font-size:11px;text-transform:uppercase;letter-spacing:0.06em;color:#64748b;font-weight:700">TOTAL</div><div style="font-size:22px;font-weight:900;color:#f43f5e;margin-top:5px">' + mlTotal.toLocaleString('en-IN') + '</div></td>' +
          '</tr></table>';
      }
    }
