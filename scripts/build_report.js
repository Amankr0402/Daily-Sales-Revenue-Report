  ...
  <div class="sec-gap">
    ...
    <div class="grid" style="grid-template-columns:1fr 1fr;gap:14px">
      ...
      <div class="chart-card" style="padding:0;overflow:hidden;display:flex;flex-direction:column;justify-content:space-between">
        ...
        <div class="clickable-card" onclick="window.open('leads-missed-details.html', '_blank')" style="margin: 12px 16px 16px; padding: 14px 18px; background: #ffffff; border: 1px solid var(--border); border-radius: 12px; text-align: center; box-shadow: 0 2px 8px rgba(0,0,0,0.02); cursor: pointer; transition: all 0.2s ease;" title="Click to view all Leads Missed details in a new tab" onmouseover="this.style.boxShadow='0 6px 16px rgba(91,62,155,0.12)';this.style.borderColor='#cbd5e1';" onmouseout="this.style.boxShadow='0 2px 8px rgba(0,0,0,0.02)';this.style.borderColor='var(--border)';">
  ...
  try {
    ...
    Object.entries(yd.sources||{}).sort((a,b)=>b[1].revenue-a[1].revenue).forEach(([src,d])=>{
      if (src === 'Direct Sale') {
        ...
        if (yd.directSaleCSV && yd.directSaleCSV.count > 0) {
          sgHtml += '<div class="src-pill clickable-pill" onclick="window.open(&apos;direct-sales-details.html?filter=yesterday&apos;, &apos;_blank&apos;)" style="border-color: rgba(236, 72, 153, 0.4); background: rgba(236, 72, 153, 0.08); cursor: pointer; transition: all 0.2s ease;" title="Click to view all Direct Sales deals from CSV in a new tab"><div class="src-dot" style="background:#ec4899;box-shadow:0 0 6px #ec489988"></div><div><div class="src-name">🌐 Total Direct Sales (CSV) <span style="font-size:11px;color:var(--primary);font-weight:800;">↗</span></div><div class="src-val">' + fmt(yd.directSaleCSV.revenue) + '</div><div class="src-cnt">' + yd.directSaleCSV.count + ' deals (Full CSV)</div></div></div>';
          if (yd.collidingTotal && yd.collidingTotal.count > 0) {
            const namesList = (yd.collidingDeals || []).map(cd => cd.customerName + ' (' + fmt(cd.revenue) + ')').join(', ');
            const fullDetails = (yd.collidingDeals || []).map(cd => cd.customerName + ' [Agent: ' + cd.agent + ']: ' + fmt(cd.revenue)).join('&#10;');
            sgHtml += '<div class="src-pill clickable-pill" onclick="window.open(&apos;direct-sales-details.html?filter=collision&apos;, &apos;_blank&apos;)" style="border-color: rgba(245, 158, 11, 0.4); background: rgba(245, 158, 11, 0.08); cursor: pointer; transition: all 0.2s ease;" title="' + fullDetails + ' (Click to view overlap deals in a new tab)"><div class="src-dot" style="background:#f59e0b;box-shadow:0 0 6px #f59e0b88"></div><div><div class="src-name">🔀 Direct &amp; Inside Overlap <span style="font-size:11px;color:var(--primary);font-weight:800;">↗</span></div><div class="src-val">' + fmt(yd.collidingTotal.revenue) + '</div><div class="src-cnt" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;max-width:210px" title="' + fullDetails + '">' + yd.collidingTotal.count + ' deals: ' + namesList + '</div></div></div>';
          }
        }
      } else {
